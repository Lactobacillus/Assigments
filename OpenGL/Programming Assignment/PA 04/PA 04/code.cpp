...
// PA #4 의 수정된 부분 #1
// Vertex Normal 과 Face Normal 을 그려주는 기능을 제어하는 변수를 만들었다.
// showFace 와 showVertex 가 그 변수이다.
// 값이 0 이면 normal 을 그리지 않고, 1 이면 normal 을 그린다.
// Directional light, Point light, Spotlight 를 제어하는 변수를 만들었다.
// directLight, pointLight, spotLight 가 그 변수이다.
// 마찬가지로 값이 0 이면 꺼진 것이고, 1 이면 켜진 것이다.
int showFace = 0;
int showVertex = 0;
int directLight = 0;
int pointLight = 0;
...
void drawCow(){
		...
		// PA #4 의 수정된 부분 #2
		// showVertex 또는 showFace 변수의 값이 1인 경우,
		// Vertex Normal 과 Face Normal 을 그린다.
		// WaveFrontOBJ 에 있는 Draw_VN 과 Draw_FN 메소드를 호출하면 된다.
		if(showVertex){
...
void Lighting(){
	...
	// PA #4 의 수정된 부분 #4
	// 3 가지 조명을 구현하였다.
	// 각 조명에 대한 변수를 지정하였고, directional light 와 spot light 의 경우,
	// 진행 방향을 나타내주기 위해 벡터 변수를 지정하였다.
	// 자세한 내용은 중간 중간에 있는 주석을 참고하면 된다.
	float direct_light_pos[] = {-10.0, 8.0, -3.0, 0.0 };
	float point_light_pos[] = {-4.0, 3.0, 7.0, 1.0 };
	float spot_light_pos[] = { 3.0, 2.0, -2.0, 1.0 };
	
	float direct_light_ambient[] = {0.3, 0.6, 0.5, 1.0};
	float direct_light_diffuse[] = { 0.4, 0.8, 0.4, 1.0 };
	float direct_light_specular[] = { 0.2, 0.7, 0.5, 1.0 };
	float point_light_ambient[] = { 0.5, 0.7, 0.9, 1.0 };
	float point_light_diffuse[] = { 0.5, 0.7, 0.9, 1.0 };
	float point_light_specular[] = { 0.5, 0.7, 0.9, 1.0 };
	float spot_light_ambient[] = { 0.9, 0.9, 0.9, 1.0 };
	float spot_light_diffuse[] = { 0.9, 0.9, 0.9, 1.0 };
	float spot_light_specular[] = { 0.9, 0.9, 0.9, 1.0 };
	float spot_light_direction[] = { 1.0, 1.0, 1.0};
	
	Vector direct_light_axis(-direct_light_pos[0], -direct_light_pos[1], -direct_light_pos[2]);
	Vector spot_light_axis(spot_light_direction[0] - spot_light_pos[0], spot_light_direction[1] - spot_light_pos[1], spot_light_direction[2] - spot_light_pos[2]);

		direct_light_axis.Normalize();
		spot_light_axis.Normalize();

		// 각 조명의 위치를 나타내는 구를 그린다.
		// directional light 와 spot light 의 경우엔 진행 방향도 나타낸다.
		// point light 의 경우, 토끼의 엉덩이 쪽에 위치한다.
		// directional light 는 노란색 진행 방향을 가진다.
		// spot light 는 핑크색 진행 방향을 가진다.
		glPushMatrix();
		glTranslatef(direct_light_pos[0], direct_light_pos[1], direct_light_pos[2]);
		glutSolidSphere(0.5, 10, 10);
		glPopMatrix();
		glDisable(GL_LIGHTING);
		glBegin(GL_LINES);
			glColor3d(1, 1, 0);
			glVertex3f(direct_light_pos[0], direct_light_pos[1], direct_light_pos[2]);
			glVertex3f(direct_light_pos[0] + 5.0 * direct_light_axis.x, direct_light_pos[1] + 5.0 * direct_light_axis.y, direct_light_pos[2] + 5.0 * direct_light_axis.z);
		glEnd();
		glEnable(GL_LIGHTING);

		glPushMatrix();
		glTranslatef(point_light_pos[0], point_light_pos[1], point_light_pos[2]);
		glutSolidSphere(0.5, 10, 10);
		glPopMatrix();

		glPushMatrix();
		glTranslatef(spot_light_pos[0], spot_light_pos[1], spot_light_pos[2]);
		glutSolidSphere(0.5, 10, 10);
		glPopMatrix();
		glDisable(GL_LIGHTING);
		glBegin(GL_LINES);
		glColor3d(1, 0, 1);
			glVertex3f(spot_light_pos[0], spot_light_pos[1], spot_light_pos[2]);
			glVertex3f(spot_light_pos[0] + 5.0 * spot_light_axis.x, spot_light_pos[1] + 5.0 * spot_light_axis.y, spot_light_pos[2] + 5.0 * spot_light_axis.z);
		glEnd();
		glEnable(GL_LIGHTING);

		// 각 조명을 관리하는 변수가 1 이면 조명을 활성화하고, 0 이면 조명을 비활성화한다.
		// 각 조명의 속성은 위에 있는 속성 변수를 이용한다.
		if(directLight){

			glEnable(GL_LIGHT1);
			glLightfv(GL_LIGHT1, GL_POSITION, direct_light_pos);
			glLightfv(GL_LIGHT1, GL_AMBIENT, direct_light_ambient);
			glLightfv(GL_LIGHT1, GL_DIFFUSE, direct_light_diffuse);
			glLightfv(GL_LIGHT1, GL_SPECULAR, direct_light_specular);
		}
		else{
		...
		if(spotLight){
			...
			glEnable(GL_LIGHT3);
			glLightfv(GL_LIGHT3, GL_POSITION, spot_light_pos);
			glLightfv(GL_LIGHT3, GL_AMBIENT, spot_light_ambient);
...
void onKeyPress(unsigned char key, int x, int y){
		...
		// 추가적으로 PA #4 에서는 쉐이딩 모델 변경도 여기로 옮겼다.
		
		// PA #4 의 수정된 부분 #5
		// shading model 변경도 여기로 옮겼다.
		// F, f 키를 누르면 Face Normal Vector 를 표시하는 기능을 설정한다. (showFace = 0 / 1)
		// N, n 키를 누르면 Vertex Normal Vector 를 표시하는 기능을 설정한다. (showVertex = 0 / 1)
		switch (key){
...
void SpecialKey(int key, int x, int y){
		...
		// PA #4 의 수정된 부분 #6
		// F1, F2, F3 키로 directional, point, spot light 를 설정한다.
		// 각 키는 GLUT_KEY_Fn 이다.
		// F1 키를 누르면 directional light 를 설정한다. (directLight = 0 / 1)
		// F2 키를 누르면 point light 를 설정한다. (pointLight = 0 / 1)
		// F3 키를 누르면 spot light 를 설정한다. (spotLight = 0 / 1)
		switch(key){
...
void main(int argc, char* argv[]){
		...
		glutCreateWindow("PA4 - 2014210085 조영우");
